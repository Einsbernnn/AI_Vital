...
<body class="bg-gradient-to-r from-green-200 to-green-400 min-h-screen flex flex-col">
    ...
    <script>
        ...
        async function fetchBPValuesAndUpdateInputs() {
            try {
                const response = await fetch('getBP.php', { cache: 'no-store' });
                const data = await response.json();
                if (data && data.systolic && data.diastolic && data.systolic !== 'NaN' && data.diastolic !== 'NaN') {
                    const systolicInput = document.getElementById('systolic');
                    const diastolicInput = document.getElementById('diastolic');
                    // Only update if values are different to avoid unnecessary events
                    if (systolicInput && diastolicInput) {
                        if (systolicInput.value !== data.systolic) {
                            systolicInput.value = data.systolic;
                        }
                        if (diastolicInput.value !== data.diastolic) {
                ...
                console.error('Error fetching BP values:', error);
            }
        }
        // Fetch BP values every 2 seconds
        setInterval(fetchBPValuesAndUpdateInputs, 2000);
        // Optionally, fetch once on page load
        fetchBPValuesAndUpdateInputs();
    </script>
</body>
</html>